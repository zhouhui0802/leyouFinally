...
/*    // Robbin学习
    ...
    public List<User> queryUserByIds(List<Long> ids) {
        ...
        String baseUrl = "http://localhost:8082/user/";

        ids.forEach(id -> {
            // 我们测试多次查询
            User user= restTemplate.getForObject(baseUrl + id, User.class);
            System.out.println("数据为:"+user.toString());
            users.add(this.restTemplate.getForObject(baseUrl + id, User.class));
            // 每次间隔500ms
            try {
                Thread.sleep(500);
    ...
    //这是最原始的写法，直接调用userDao里面的模式，通过写死的URL直接访问
    @Autowired
    ...
    //第三种调用方式
    public List<User> queryUserByIdsLoadbalance(List<Long> ids) {
        List<User> users = new ArrayList<>();
        // 地址直接写服务名称即可
    ...
    //使用hystrix去回滚访问
    public List<User> queryUserByIdsHystrix(List<Long> ids) throws Exception{
        List<User> users = new ArrayList<>();
        for (Long id : ids) {
            //User user = this.userDao.queryUserById(id);
