...
public class SearchServiceImpl implements SearchService {
    ...
    public Goods buildGoods(Spu spu) throws IOException {
        ...
        // 关键：必须加 try-catch，防止 Feign 阻塞卡死
        SpuDetail spuDetail = null;
        try {
            ResponseEntity<SpuDetail> responseEntity = this.goodsClient.querySpuDetailBySpuId(spu.getId());
            if (responseEntity != null) {
                spuDetail = responseEntity.getBody();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
